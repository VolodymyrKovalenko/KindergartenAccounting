...
mainTable.onclick = function(event){
 ...
 let tableWithButtons = document.getElementsByClassName('tableWithButtons')[0];
        if(tableWithButtons){
			for(let i=0;i<tableWithButtons.tBodies[0].rows.length;i++){
				if(tableWithButtons.tBodies[0].rows[i].classList.contains('workers')) continue;
				if(tableWithButtons.tBodies[0].rows[i].cells[0].innerHTML != '0'){
					let deleteButton = tableWithButtons.tBodies[0].rows[i].getElementsByClassName('delete')[0];
